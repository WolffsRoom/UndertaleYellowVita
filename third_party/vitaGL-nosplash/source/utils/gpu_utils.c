...
static inline __attribute__((always_inline)) void *gpu_alloc_mapped_aligned_for_cpu_inner(size_t alignment, size_t size) {
	// Keep the failure result deterministic. When every mapped pool is full
	// and external memory is disabled, the old code returned an uninitialised
	// stack value which could look like a valid texture address.
	void *res = NULL;
...
static inline __attribute__((always_inline)) void *gpu_alloc_mapped_aligned_for_gpu_inner(size_t alignment, size_t size) {
	// See the CPU variant above: callers must receive NULL on exhaustion so
	// they can defer the upload instead of writing through a garbage pointer.
	void *res = NULL;
	vgl_alloc_attempt(alignment, size, VGL_MEM_VRAM)
	vgl_alloc_attempt(alignment, size, VGL_MEM_RAM)
	vgl_alloc_attempt(alignment, size, VGL_MEM_SLOW)
...
void *gpu_alloc_mapped_for_texture(size_t size) {
    void *res = NULL;
    // Chapter 5 still filled CDRAM after room construction because only the
    // first few BC3 pages could use RAM/PHY; the old 8/4 MiB reserves then
    // redirected every late page back to VRAM. Keep smaller operational
    // reserves and allow two more 4 MiB atlases to remain outside CDRAM.
    const size_t ram_reserve = 3 * 1024 * 1024;
    const size_t slow_reserve = 1 * 1024 * 1024;
    // BC3 is sampled by GXM from every mapped pool. Use the otherwise-idle
    // pools first, but leave enough space for transient uploads/audio and room
    // construction. The ordinary GPU allocator remains VRAM-first for display
    // buffers, render targets and application surfaces.
    if (vglMemFree(VGL_MEM_RAM) > size + ram_reserve) {
        res = vgl_memalign(MEM_ALIGNMENT, size, VGL_MEM_RAM);
        if (res) return res;
    }
    if (vglMemFree(VGL_MEM_SLOW) > size + slow_reserve) {
        res = vgl_memalign(MEM_ALIGNMENT, size, VGL_MEM_SLOW);
        if (res) return res;
    }
    res = vgl_memalign(MEM_ALIGNMENT, size, VGL_MEM_VRAM);
    if (res) return res;
    // If VRAM is fragmented, use the remaining mapped pools as a last resort.
    res = vgl_memalign(MEM_ALIGNMENT, size, VGL_MEM_RAM);
    if (res) return res;
    res = vgl_memalign(MEM_ALIGNMENT, size, VGL_MEM_SLOW);
    if (res) return res;
    if (use_extra_mem) return vgl_memalign(MEM_ALIGNMENT, size, VGL_MEM_EXTERNAL);
    return NULL;
}
...
void gpu_alloc_texture(uint32_t w, uint32_t h, SceGxmTextureFormat format, const void *data, texture *tex, uint8_t src_bpp, uint32_t (*read_cb)(void *), void (*write_cb)(void *, uint32_t), GLboolean fast_store) {
    ...
    // Large immutable atlas uploads do not need to consume CDRAM. GXM can
    // sample them directly from mapped RAM/PHY, while render-target textures
    // (data == NULL) retain the VRAM-first path. This prevents Chapter 5's
    // lossless RGBA4444/font pages from filling CDRAM even though the BC3
    // pages already use the distributed texture allocator.
    void *texture_data = data != NULL && tex_size >= 512 * 1024 ?
                         gpu_alloc_mapped_for_texture(tex_size) :
                         gpu_alloc_mapped_for_gpu(tex_size);

    // The unsafe generic allocator can otherwise return from an exhausted
    // room transition with no valid backing store and the conversion loop
    // writes through an invalid pointer. Report OOM to glTexImage2D's caller;
    // the runner will defer and retry this page on a later frame.
    if (!texture_data) {
		...
		if (data) {
			if (uncompressed) {
				...
				case SCE_GXM_TEXTURE_FORMAT_PVRT4BPP_ABGR:
					...
					break;
				case SCE_GXM_TEXTURE_FORMAT_UBC3_ABGR:
					// The GXM transfer path occasionally produced malformed BC3
					// block ordering for 2048x2048 GameMaker atlases. Use the
					// deterministic CPU swizzler for BC3; this runs only once at
					// upload and avoids corrupting every sprite sharing the page.
					SwizzleTexData128Bpp((uint8_t *)mip_data, (uint8_t *)data,
						0, 0, ALIGNBLOCK(w, 4), ALIGNBLOCK(h, 4), ALIGNBLOCK(w, 4),
						ALIGNBLOCK(MIN(aligned_width, aligned_height), 4));
					break;
				case SCE_GXM_TEXTURE_FORMAT_UBC2_ABGR:
				case SCE_GXM_TEXTURE_FORMAT_UBC5_GR:
	...
	if (mip_level) {
		...
		if (mip_count >= mip_level)
			...
			if (!texture_data) {
				...
				texture_data = gpu_alloc_mapped_for_texture(tex_size);
		...
		if (data) {
			if (uncompressed) {
				...
				case SCE_GXM_TEXTURE_FORMAT_PVRT4BPP_ABGR:
					...
					break;
				case SCE_GXM_TEXTURE_FORMAT_UBC3_ABGR:
					// See the matching base-level path above. BC3 atlases use a
					// CPU swizzle so their block layout is stable on real hardware.
					SwizzleTexData128Bpp((uint8_t *)mip_data, (uint8_t *)data,
						0, 0, ALIGNBLOCK(w, 4), ALIGNBLOCK(h, 4), ALIGNBLOCK(w, 4),
						ALIGNBLOCK(MIN(aligned_width, aligned_height), 4));
					break;
