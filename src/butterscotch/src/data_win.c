...
#ifdef PLATFORM_VITA
#include <pthread.h>
static pthread_mutex_t vitaLazyFileMutex = PTHREAD_MUTEX_INITIALIZER;
#endif
...
void DataWin_loadTxtrIfNeeded(DataWin* dw, uint32_t textureId) {
    ...
    // Texture, room and audio lazy loads share the same FILE cursor. A room
    // transition can overlap an audio worker read; without a stdio lock the
    // saved cursor may be restored in the middle of the TXTR read, producing
    // a valid-looking but truncated PNG and subsequent GPU corruption.
#ifdef PLATFORM_VITA
    pthread_mutex_lock(&vitaLazyFileMutex);
#endif
    long old_seek = ftell(dw->lazyLoadFile);
    clearerr(dw->lazyLoadFile);
...
#ifdef PLATFORM_VITA
    pthread_mutex_unlock(&vitaLazyFileMutex);
#endif
