...
static inline PreprocessedText TextUtils_preprocessGmlTextIfNeeded(Runner* runner, const char* text) {
    if (text != nullptr &&
        (strstr(text, "{PORT_VERSION}") != nullptr ||
         strstr(text, "v0.68 - Wolff") != nullptr ||
         strstr(text, "DELTARUNE v0.68 (Internal Build)") != nullptr ||
         strstr(text, "DELTARUNE VITA v0.69") != nullptr ||
         strstr(text, "DELTARUNE VITA v0.70") != nullptr)) {
        PreprocessedText ret = {0};
        char version[96] = "UNDERTALE YELLOW VITA v0.1";
        FILE* versionFile = fopen("app0:assets/build_version.txt", "rb");
        if (versionFile != nullptr) {
            size_t count = fread(version, 1, sizeof(version) - 1, versionFile);
            fclose(versionFile);
            version[count] = '\0';
            while (count > 0 && (version[count - 1] == '\r' || version[count - 1] == '\n' || version[count - 1] == ' '))
                version[--count] = '\0';
        }
        ret.text = safeStrdup(version);
        ret.owning = true;
        return ret;
    }
...
#ifdef PLATFORM_VITA
    if (text != nullptr) {
        if (strstr(text, "Press Z or ENTER") != nullptr ||
            strstr(text, "Press Z or Y") != nullptr ||
            strstr(text, "press Z or ENTER") != nullptr ||
            strstr(text, "Pressione Z ou ENTER") != nullptr ||
            strstr(text, "Pulsa Z o ENTER") != nullptr ||
            strstr(text, "CONFIRM - [Z, ENTER]::") != nullptr ||
            strstr(text, "CANCEL  - [X, SHIFT]::") != nullptr ||
            strstr(text, "MENU    - [C, CTRL] ::") != nullptr) {
            
            char buffer[256];
            snprintf(buffer, sizeof(buffer), "%s", text);
            char* hit = strstr(buffer, "Press Z or ENTER");
            if (hit) memcpy(hit, "Press [X] button", 16);
            hit = strstr(buffer, "Press Z or Y");
            if (hit) memcpy(hit, "Press [X]   ", 12);
            hit = strstr(buffer, "press Z or ENTER");
            if (hit) memcpy(hit, "press [X] button", 16);
            hit = strstr(buffer, "Pressione Z ou ENTER");
            if (hit) memcpy(hit, "Pressione [X]       ", 20);
            hit = strstr(buffer, "Pulsa Z o ENTER");
            if (hit) memcpy(hit, "Pulsa [X]      ", 15);
            hit = strstr(buffer, "CONFIRM - [Z, ENTER]::");
            if (hit) memcpy(hit, "CONFIRM - [X]       ::", 22);
            hit = strstr(buffer, "CANCEL  - [X, SHIFT]::");
            if (hit) memcpy(hit, "CANCEL  - [O]       ::", 22);
            hit = strstr(buffer, "MENU    - [C, CTRL] ::");
            if (hit) memcpy(hit, "MENU    - [TRIANGLE]::", 22);

            PreprocessedText ret = {0};
            ret.text = safeStrdup(buffer);
            ret.owning = true;
            return ret;
        }
    }
...
static inline void TextUtils_trimTrailingWhitespace(char* str) {
    if (str == nullptr) return;
    size_t len = strlen(str);
    while (len > 0 && TextUtils_isWhitespaceChar(str[len - 1])) {
        str[--len] = '\0';
    }
}
...
static inline PreprocessedText TextUtils_wrapText(Font* font, const char* text, int32_t maxWidth) {
    ...
    while (len > lineStart) {
        ...
        while (len > lineEnd && !TextUtils_isNewlineChar(text[lineEnd])) lineEnd++;

        int32_t currentStart = lineStart;
        while (lineEnd > currentStart) {
            float width = 0;
            int32_t bestBreak = -1;
            int32_t pos = currentStart;

            while (lineEnd > pos) {
                int32_t tentative = pos;
                uint16_t cp = TextUtils_decodeUtf8(text, len, &tentative);
                FontGlyph* glyph = TextUtils_findGlyph(font, cp);

                float charWidth = glyph != nullptr ? glyph->shift : 0;
                if (tentative < lineEnd) {
                    int32_t nextPos = tentative;
                    uint16_t nextCp = TextUtils_decodeUtf8(text, len, &nextPos);
                    if (glyph != nullptr) charWidth += TextUtils_getKerningOffset(glyph, nextCp);
                }

                if (width + charWidth > (float) maxWidth && pos > currentStart) break;

                width += charWidth;
                pos = tentative;

                if (pos < lineEnd && TextUtils_isWhitespaceChar(text[pos])) bestBreak = pos;
            }

            int32_t segmentEnd = pos;
            if (lineEnd > pos && bestBreak > currentStart) segmentEnd = bestBreak;

            if (outLen > 0) out[outLen++] = '\n';
            memcpy(out + outLen, text + currentStart, segmentEnd - currentStart);
            outLen += segmentEnd - currentStart;

            currentStart = segmentEnd;
            while (lineEnd > currentStart && TextUtils_isWhitespaceChar(text[currentStart])) currentStart++;
        }
