...
#define VITA_PVR_ROOT "ux0:data/deltarune/deltarunevita/pvr"
...
// Translated data.win variants repack the Latin fonts and dialogue frame
// sprites into pages that do not necessarily match the official layout.
// BC3 is safe for scenery, but its block interpolation can alter thin glyphs
// and the white textbox pixels. Detect those pages from the active data.win
// instead of maintaining chapter-specific page numbers.
...
static bool vitaPixelCriticalPage(const DataWin* dw, uint32_t pageId) {
    ...
    if (vitaTranslatedUiRequiresLossless(dw, pageId)) return true;
...
static bool vitaPvrRequiresLosslessCompanion(const DataWin* dw, uint32_t pageId) {
    ...
    snprintf(markerPath, sizeof(markerPath),
             VITA_PVR_ROOT "/chapter%d%s/page_%03u.rgba4444",
    ...
    return pageId == 12U || pageId == 14U || pageId == 16U || pageId == 17U ||
...
static bool vitaPvrTextureAllowed(const DataWin* dw, uint32_t pageId) {
    ...
    // Profile 0: Otimizado (Optimized) -> BC3 for 2048x2048 atlases, RGBA4444 for <=1024
    if (g_vitaTextureFormatProfile == 0) {
        const Texture* txtr = &dw->txtr.textures[pageId];
        if (txtr != nullptr && (txtr->textureWidth >= 2048 || txtr->textureHeight >= 2048 || txtr->blobSize >= 500000)) {
            return true;
        }
        return false;
    }
...
static void vitaPvrPath(const DataWin* dw, uint32_t pageId, char* output, size_t outputSize) {
    const char* suffix = (g_vitaBc3OnlyEnabled ||
                          (vitaCh2TeacupBc3Mode && vitaTextureChapter(dw) == 2 &&
                           vitaCh2TeacupBc3Page(pageId))) ? ".bc3.pvr" : ".pvr";
    const char* variant = vitaTextureCacheVariant(dw);
    if (strcmp(variant, "-ptbr") == 0)
        snprintf(output, outputSize, "ux0:data/deltarune/deltarunevita/mods/Lang/Portuguese-BR/pvr/chapter%d/page_%03u%s",
                 vitaTextureChapter(dw), pageId, suffix);
    else if (strcmp(variant, "-spa") == 0)
        snprintf(output, outputSize, "ux0:data/deltarune/deltarunevita/mods/Lang/Spanish/pvr/chapter%d/page_%03u%s",
                 vitaTextureChapter(dw), pageId, suffix);
    else if (strcmp(variant, "-ita") == 0)
        snprintf(output, outputSize, "ux0:data/deltarune/deltarunevita/mods/Lang/Italian/pvr/chapter%d/page_%03u%s",
                 vitaTextureChapter(dw), pageId, suffix);
    else if (strcmp(variant, "-tur") == 0)
        snprintf(output, outputSize, "ux0:data/deltarune/deltarunevita/mods/Lang/Turkish/pvr/chapter%d/page_%03u%s",
                 vitaTextureChapter(dw), pageId, suffix);
    else
        snprintf(output, outputSize, VITA_PVR_ROOT "/chapter%d/page_%03u%s",
                 vitaTextureChapter(dw), pageId, suffix);
}
...
static bool vitaVariantPageDiffers(const DataWin* dw, uint32_t pageId) {
    ...
    // Spanish ships independently repacked atlases. Never borrow an official
    // page by index when its own PVR is absent.
    if (strcmp(variant, "-spa") == 0) return true;
    int chapter = vitaTextureChapter(dw);
    // Pixel-level diff against the official atlas (not just file hash, which
    // false-positives on PNG re-encoding) found real content differences well
    // beyond the originally curated pages - up to 70% of a page's pixels for
    // Chapter 3 page 23. Pages 27-30 exist only in the PT-BR atlas (repacking
    // spillover from longer translated text), so they always need the -ptbr
    // PVR and can never fall back to the base chapter2 directory.
    if (chapter == 2)
        return pageId >= 16U;
    if (chapter == 3)
        return (pageId >= 13U && pageId <= 29U) || pageId >= 33U;
    // The PT-BR chapter1/chapter4 data.win files got their whole texture
    // atlas repacked (not just the translated pages), so every page differs
    // byte-for-byte from the official build - unlike chapter2/3 above, there
    // is no shared subset worth special-casing here.
    if (chapter == 1)
        return pageId <= 10U;
    if (chapter == 4)
        return pageId <= 46U;
...
static bool vitaTextureCacheIsComplete(const DataWin* dw) {
    ...
    // Older completion markers counted PVR-only pages as prepared. If the
    // user switches to the lossless RGBA4444 profile those pages are absent,
    // despite the marker claiming a complete set. Validate lightweight file
    // headers here so only the missing pages are rebuilt during loading.
    for (uint32_t pageId = 0; pageId < dw->txtr.count; ++pageId) {
        if (vitaPvrTextureIsAvailable(dw, pageId) &&
            !vitaPvrRequiresLosslessCompanion(dw, pageId)) continue;
        if (!vitaPreparedTextureExists(dw, pageId)) return false;
    }
    return true;
}
...
static void glTextureSetStage(Renderer* renderer, int32_t slot, uint32_t texHandle) {
    ...
    glUniform1i(slot, slot);            // tell the shader to read sampler from GL_TEXTURE0+slot
...
static void glCompileAllShaders(GLLegacyRenderer* gl) {
    ...
    for (uint32_t shaderIndex = 0; shaderIndex < dw->shdr.count; shaderIndex++) {
        ...
        if (vsOk != GL_TRUE) {
            ...
            sceClibPrintf("[SHDR] vertex failed %u (%s): %s\n", shaderIndex, sh->name ? sh->name : "?", info);
#endif
            glDeleteShader(vs);
            continue;
        }
        ...
        if (fsOk != GL_TRUE) {
            ...
            sceClibPrintf("[SHDR] fragment failed %u (%s): %s\n", shaderIndex, sh->name ? sh->name : "?", info);
#endif
            glDeleteShader(vs);
            glDeleteShader(fs);
            continue;
        ...
        if (linkOk != GL_TRUE) {
            ...
            sceClibPrintf("[SHDR] link failed %u (%s): %s\n", shaderIndex, sh->name ? sh->name : "?", info);
#endif
...
static void glGpuSetShader(Renderer* renderer, MAYBE_UNUSED int32_t shaderIndex) {
    // In legacy fixed-pipeline renderer (immediate mode glVertex2f), binding custom GLSL
    // programs via glUseProgram causes attribute location mismatches that make sprites
    // and enemies render as 0-size / invisible. Always use fixed pipeline in legacy-gl.
    glUseProgram(0);
}
